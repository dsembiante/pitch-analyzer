...
The DataFrame has columns (frame, timestamp_s, separation_deg) covering every
valid (non-excluded) frame in the window, with the smoothed separation values.
An empty DataFrame with the same columns is returned on failure so callers can
check df.empty without special-casing.
"""
...
_EMPTY_SERIES = pd.DataFrame(columns=["frame", "timestamp_s", "separation_deg"])
...
def compute(
    ...
    """Compute maximum hip-shoulder separation and per-frame time series.

    Returns:
        (MetricResult, series_df) where series_df has columns
        (frame, timestamp_s, separation_deg) using smoothed values.
        Returns an empty series DataFrame on failure.
    """
    ...
    series_df = pd.DataFrame({
        "frame":          frame_of_sep,
        "timestamp_s":    [f / fps for f in frame_of_sep],
        "separation_deg": smoothed.tolist(),
    })
    ...
    metric = MetricResult(
        ...
        unit="degrees",
        description="Peak torso coil: max angular gap between hip and shoulder lines during windup. Larger = more elastic energy stored.",
        frame=max_frame,
        phase=_PHASE,
        notes=(
